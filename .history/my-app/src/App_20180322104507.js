...
class App extends Component {
  ...
  toggleCheckbox = checkbox => {
    ...
    if (this.selectedCheckboxes.size === 1){
      listClassNames.concat(styles.border);
      this.setState({
      ...
      listClassNames.concat(styles.backgroundBorder);
      this.setState({
        classNames: listClassNames
      })
        console.log(listClassNames);
      ...
      <div>
        {items.map(this.createCheckbox)}
        {
          Card.list.map(function(i){
            if (i%3) {
              component = <Card key={i} cardNumber={number} className={classNames}/> 
              number++
            } else {
