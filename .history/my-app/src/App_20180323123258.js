...
class App extends Component {
  ...
  toggleCheckbox = (label, state) => {
    this.selectedCheckboxes.has(label) ? 
      ...
      if(state && this.selectedCheckboxes.size === 1) {
        console.log(Array.from(this.state.classNames))
        var foo = Array.from(this.state.classNames).pop()
        this.setState(prevState => ({
          classNames: prevState.classNames.delete(foo)
        }));
        // this.setState({classNames: Array.from(this.state.classNames).splice(-1,1)})
        console.log(Array.from(this.state.classNames) + ' after')
      }
